...
- 미지의 클래스 요소·**맨 span**(`.chk` 플렉스 래퍼)은 구조 컨테이너로 원형 보존(`chain`). 절대 벗기지 말 것. 단, **스팬의 `data-*` 속성은 흡수 대상** — 엔진 밖 속성으로 스타일을 숨기면 "일괄 수정해도 그 부분만 멈추는" 버그가 된다 (L22).
- 선택과 "경계만 닿은" 문단·스팬은 건드리지 않는다 (`parasInRange`의 `end > start` 필터).
- 진입점은 `applyInline(patch[, paraFn])`(인라인)·`applyBlockStyle(fn)`(문단: 정렬·줄간격·자간)뿐. DOM Range를 직접 수술하지 말 것. 두 진입점 모두 **호스트(필드) 경계를 넘어 동작**하고(`hostsInRange`), 일괄 적용은 복합 undo 엔트리(`{items:[...]}`) 하나로 원자화된다.
- cover 필드처럼 호스트 직속에 블록 스타일을 걸어야 하면 `<div data-line>`으로 감싼 뒤 적용 (writeBack이 innerHTML만 저장하므로). 래핑은 멱등이어야 하고, `normalizeField`가 span>div·중복 data-line 비정상 구조를 평탄화한다 (렌더 시에도 실행).
- 블록 서식 적용 후에는 선택을 글자 오프셋으로 복원한다 (DOM 이동으로 Range가 무너지는 연쇄 방지).
...
[7]  PPT식 서식 바 — 2줄 (selectionState/updateFmtState · 색 피커: 테마 음영/표준/최근/스포이트 · 단축키)
[8]  이모지 (데이터·패널·삽입)
